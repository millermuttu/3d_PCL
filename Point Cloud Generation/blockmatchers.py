...
class StereoBM(BlockMatcher):
    ...
    def blockSize(self, value):
        ...
        if (value > 4 and
            value < self.parameter_maxima["blockSize"] and
            value % 2):
    ...
    def _replace_bm(self):
        ...
        self._block_matcher = cv2.StereoBM_create(numDisparities=self.num_disp,blockSize=self.block_size)
...
class StereoSGBM(BlockMatcher):
    ...
    def __init__(self, min_disparity=2, num_disp=16, sad_window_size=3,
                 uniqueness=10, speckle_window_size=100, speckle_range=32,
                 p1=216, p2=864, max_disparity=5,
                 settings=None):
