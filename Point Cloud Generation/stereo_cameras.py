...
class StereoPair(object):
    ...
    def __init__(self, devices):
        ...
        if devices[0] != devices[1]:
            ...
            self.captures[0].set(3, 640)
            self.captures[0].set(4, 480)

            self.captures[1].set(3, 640)
            self.captures[1].set(4, 480)
        else:
    ...
    def get_chessboard(self, columns, rows, show=False):
        ...
        while not all(found_chessboard):
            ...
            for i, frame in enumerate(frames):
                (found_chessboard[i],
                 corners) = cv2.findChessboardCorners(frame, (columns, rows), flags=cv2.CALIB_CB_FAST_CHECK)
        return frames
...
class CalibratedPair(StereoPair):
    ...
    def get_point_cloud(self, pair):
        ...
        colors = cv2.cvtColor(pair[1], cv2.COLOR_BGR2RGB)
